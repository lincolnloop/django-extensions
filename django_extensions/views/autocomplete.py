...
def manytomany_autocomplete(request, related_string_functions=None):
    """
    Searches in the fields of the given related model and returns the 
    result as a simple string to be used by the jQuery Autocomplete plugin.
    
    This has a special case lookup if the GET['object_pk'] contains commas we 
    do a bulk lookup.
    """
    if related_string_functions is None:
        related_string_functions = getattr(settings,
    ...
    if search_fields and app_label and model_name and (query or object_pk):
        ...
        if query:
            ...
            print object_pk
            try:
                objs = queryset.in_bulk(object_pk.split(','))
                print objs
            except:
                pass
            else:
                data = ''.join([u'%s|%s\n' % (
                    to_string_function(v), k) for k,v in objs.iteritems()])
        return HttpResponse(data)
    return HttpResponseNotFound()
